...
class Package
{
    ...
    public static function isLocked($package)
    {
        ...
        if (isset($package['password']) && $package['password'] != '') {
            $lock = 'locked';
        }
    ...
    public static function userCanAccess($ID, $type = 'package')
    {
        ...
        if ($type=='package') {
            ...
            $roles = \WPDM\libs\CategoryHandler::GetAllowedRoles($ID);
        }
...
//isset($package['password_lock']) && (int)$package['password_lock'] == 1 &&
    ...
    {
        if (self::isLocked($ID) && !isset($_SESSION['_wpdm_unlocked_'.$ID])) {
            return '#locked';
        }
        if ($ext) {
            $ext = '&' . $ext;
    ...
    public static function fetchTemplate($template, $vars, $type = 'link')
    {
        ...
        if (isset($templates[$template]) && isset($templates[$template]['content'])) {
        ...
        } else if (!strpos(strip_tags($template), "]")) {
            ...
            } else if (file_exists($ltpldir . $template)) {
                $template = file_get_contents($ltpldir . $template);
            } else if (file_exists($ltpldir . $template . '.php')) {
                $template = file_get_contents($ltpldir . $template . '.php');
            } else if (file_exists($ltpldir. $type . "-template-" . $template . '.php')) {
                $template = file_get_contents($ltpldir. $type . "-template-" . $template . '.php');
            } else {
                $template = file_get_contents(wpdm_tpl_path($default[$type], $ltpldir));
            }
        }
